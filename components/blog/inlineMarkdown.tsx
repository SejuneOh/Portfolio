...
function renderMatch(kind: Kind, m: RegExpMatchArray, key: number): React.ReactNode {
  /*
    ...
    **이 채움은 상자를 만들지 못한다.** 지면과 같은 색이라 CSS 상 명암비 1.0:1 이다.
    불투명이라 뒤에 깔린 계측 캔버스를 가리기는 하는데, 뒤에 격자선이 지나가는 자리에서만
    자국처럼 읽히고 빈 자리에서는 보이지 않는다 — 프리뷰 픽셀로 재니 최악 자리가 1.005:1
    이었다. 위치에 따라 달라지므로 기댈 수 없다.
    ...
    같은 자리를 큰 상자에서는 이미 고쳤다 — 거기서는 채움이 아무 일도 못 하는 것이
    그대로 손해였다.
      app/(site)/about/page.tsx:161          Contact 카드 → card
      app/(site)/writing/[slug]/page.tsx:145 TL;DR → 라임 좌측 눈금
