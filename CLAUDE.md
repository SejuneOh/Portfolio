...
| `.claude/skills/issue-review/` | 그 PR을 검사한다 | `.claude/worktrees/review-<N>` (detach) | **코멘트만** |

둘은 **서로 다른 세션**에서 돈다. 같은 세션이면 자기가 쓴 코드를 자기가 검사하게 되어
검증이 성립하지 않는다. 작업 공간도 분리돼 있어 서로의 파일을 덮어쓰지 않는다.
